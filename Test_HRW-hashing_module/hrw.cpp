...
int main() {
    vector<string> strings_with_id = {"server1"};

    string key= "Secret2";
    vector<pair<string, uint32_t>> ordered_ids = order_HRW(strings_with_id, key);

    // Print the ordered IDs and their associated hashes
    for (const auto& pair : ordered_ids) {
        std::cout << "ID: " << pair.first << ", Hash: " << pair.second << std::endl;
    }

    vector<int> result = convert_strings_with_id_to_ids(strings_with_id);
    for (int id: result){
        std::cout << id << " " ;
    }
