...
static sgx_status_t SGX_CDECL sgx_ecall_put(void* pms)
{
	...
	size_t _len_key = 200 * sizeof(char);
	char* _in_key = NULL;
	char* _tmp_value = __in_ms.ms_value;
	size_t _len_value = 200 * sizeof(char);
	char* _in_value = NULL;
...
static sgx_status_t SGX_CDECL sgx_ecall_get(void* pms)
{
	...
	if (_tmp_value != NULL && _len_value != 0) {
		...
		if ((_in_value = (char**)malloc(_len_value)) == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}
