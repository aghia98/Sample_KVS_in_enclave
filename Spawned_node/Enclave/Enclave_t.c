...
static sgx_status_t SGX_CDECL sgx_ecall_add_lost_keys(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_ecall_add_lost_keys_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_ecall_add_lost_keys_t* ms = SGX_CAST(ms_ecall_add_lost_keys_t*, pms);
	ms_ecall_add_lost_keys_t __in_ms;
	if (memcpy_s(&__in_ms, sizeof(ms_ecall_add_lost_keys_t), ms, sizeof(ms_ecall_add_lost_keys_t))) {
		return SGX_ERROR_UNEXPECTED;
	}
	sgx_status_t status = SGX_SUCCESS;
	char* _tmp_keys_with_last_share_owner = __in_ms.ms_keys_with_last_share_owner;
	size_t _len_keys_with_last_share_owner = 10000 * sizeof(char);
	char* _in_keys_with_last_share_owner = NULL;

	CHECK_UNIQUE_POINTER(_tmp_keys_with_last_share_owner, _len_keys_with_last_share_owner);
	...
	if (_tmp_keys_with_last_share_owner != NULL && _len_keys_with_last_share_owner != 0) {
		if ( _len_keys_with_last_share_owner % sizeof(*_tmp_keys_with_last_share_owner) != 0)
		{
			status = SGX_ERROR_INVALID_PARAMETER;
			goto err;
		}
		_in_keys_with_last_share_owner = (char*)malloc(_len_keys_with_last_share_owner);
		if (_in_keys_with_last_share_owner == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		if (memcpy_s(_in_keys_with_last_share_owner, _len_keys_with_last_share_owner, _tmp_keys_with_last_share_owner, _len_keys_with_last_share_owner)) {
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}

	}
	ecall_add_lost_keys(_in_keys_with_last_share_owner);

err:
	if (_in_keys_with_last_share_owner) free(_in_keys_with_last_share_owner);
	return status;
}
...
SGX_EXTERNC const struct {
	...
	5,
	{
		...
		{(void*)(uintptr_t)sgx_ecall_add_lost_keys, 0, 0},
	}
};
